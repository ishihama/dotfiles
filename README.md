...
- **サイドバー** (`prefix+a`): 全Claudeセッションの状態・経過時間・最後のプロンプト (=何の作業か) を一覧表示。入力待ちのセッションは通知メッセージ、待機中のセッションは**Claudeの最後の返答** (=何と言って終わったか、トランスクリプトから抽出) も併記。同一tmuxセッション内に複数のClaudeがいる場合も `セッション名 window.pane` 表記で区別できる。開くとフォーカスがサイドバーに移り、数字キー `1-9` でそのClaudeのpaneにジャンプ (ジャンプ後サイドバーは自動で閉じる)、`q` または再度 `prefix+a` で閉じる
