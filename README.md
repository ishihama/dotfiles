...
- **サイドバー** (`prefix+a`): 全Claudeセッションの状態・経過時間・最後のプロンプト/通知を一覧表示。開くとフォーカスがサイドバーに移り、数字キー `1-9` でそのセッションにジャンプ、`q` または再度 `prefix+a` で閉じる
- **ジャンプ** (`prefix+u`): 最優先のClaudeセッション (🔔入力待ち優先) に一発で切り替え。対象セッションを別のGhosttyウィンドウで表示中ならそのウィンドウごと前面化 (初回はmacOSのオートメーション許可が必要、`CLAUDE_JUMP_FOCUS_DISABLED=1` で無効化)。サイドバーの数字キージャンプも同様
