...
### Claude Code状態表示 (herdr風)

Claude Codeのhooksが各セッションの状態を `~/.local/state/claude/sessions/` に記録し、tmuxに表示する。

- **ステータスバー**: 状態別カウントを常時表示。hooks経由で即時更新
- **サイドバー** (`prefix+a`): 全Claudeセッションの状態・経過時間・最後のプロンプト (=何の作業か) を一覧表示。入力待ちのセッションは通知メッセージ、待機中のセッションは**Claudeの最後の返答** (=何と言って終わったか、トランスクリプトから抽出) も併記。同一tmuxセッション内に複数のClaudeがいる場合も `セッション名 window.pane` 表記で区別できる。最後のプロンプトが「続けて」のように短い場合はセッション最初のプロンプト (=本題) を表示する。開くとフォーカスがサイドバーに移り、数字キー `1-9` でそのClaudeのpaneにジャンプ (ジャンプ後サイドバーは自動で閉じる)、`i`→数字で詳細ポップアップ (cwd・ブランチ・プロンプト・直近の会話数行)、`q` または再度 `prefix+a` で閉じる
...
状態の意味:
- 🔔 入力待ち (許可プロンプトなどでブロック中) — 対応が必要
- ⚙ 作業中 (プロンプト処理中)
- ✓ 待機 (応答完了、次の指示待ち)

herdrと違い画面パースではなくhooksベースなので、状態検知は正確。
