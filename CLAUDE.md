...
- Claude Code hooks (`.config/claude/hooks/update-state.sh`) record each session's state (working/waiting/idle) plus its tmux session/window/pane to `~/.local/state/claude/sessions/*.json`. Dialog approvals (plan/permission) have no hook event of their own, so a PostToolUse hook flips waiting back to working when the first tool runs after approval
