...
│   │   ├── gh-wrapper.sh       # gh CLI auto account switch
...
- `~/.config/gh/account-map` - Generated by `scripts/setup/git.sh` (gh CLI account mapping)
...
- Claude Code hooks (`.config/claude/hooks/update-state.sh`) record each session's state (working/waiting/idle) plus its tmux session/window/pane to `~/.local/state/claude/sessions/*.json`. Dialog approvals (plan/permission) have no hook event of their own, so a PostToolUse hook (`tool_use`) flips waiting back to working when the first tool runs after approval. That hook fires on *every* tool call, so it exits before taking the state lock when the session is already `working` — keep that early return if you touch it
...
- `prefix+a` toggles a sidebar pane (`.config/shell/bin/claude-sidebar-toggle`) listing all Claude sessions with state glyph (🔔 waiting for input / ⚙ working / ✓ idle), age, `window.pane` location (distinguishes multiple Claude instances inside one tmux session), and last prompt (what the task is; waiting sessions also show the notification message, idle sessions show Claude's final reply extracted from the transcript); keys `1-9` jump to that session's pane (the sidebar closes itself after a jump), `i`+digit opens a detail popup (cwd, branch, first/last prompt, recent conversation from the transcript), `q` closes it. When the last prompt is short (e.g. "続けて", <16 chars) the session's first prompt is shown instead
...
- Setup script checks for SSH keys; if missing, it offers to generate them (ssh-keygen) and register them to GitHub (gh ssh-key add, with gh auth login / scope-refresh fallbacks). This also runs on re-runs when Git config already exists but keys are missing
- **gh CLI auto-switch**: `.config/shell/gh-wrapper.sh` wraps `gh` to auto-set `GH_TOKEN` based on `~/repos/github.com/{owner}/` directory
- `~/.config/gh/account-map` maps directory owners to gh usernames (auto-generated by `scripts/setup/git.sh`, NOT git-managed)
- When owner matches gh username (e.g. personal), no mapping entry needed (falls back to owner name)
- When owner differs from gh username (e.g. work org), the map provides the translation
